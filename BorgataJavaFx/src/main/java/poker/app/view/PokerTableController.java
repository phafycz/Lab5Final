...
public class PokerTableController {
	...
	private int cardDrawPlayer = 0;
	private int cardDrawCommon = 0;
	@FXML
	...
	private void handleDraw() {
		...
		Action act = new Action(gme, cardDrawPlayer, cardDrawCommon, draw);

		if (act.geteDrawAction() == eDrawAction.DrawPlayer) {
			// Draw a card for each player seated
			for (int iDraw = 0; iDraw < act.getCardDraw(); iDraw++) {
				cardDrawPlayer++;
				for (Player p : mainApp.GetSeatedPlayers()) {
					...
					tranDealCards.getChildren().add(CalculateTransition(c, PlayerCardBox, imView, cardDrawPlayer));
				}
			}
			...
			for (int iDraw = 0; iDraw < act.getCardDraw(); iDraw++) {
				...
				tranDealCards.getChildren().add(CalculateTransition(c, HboxCommunityCards, imView, cardDrawCommon));
			}
		}
		...
		if (act.isbEvaluateHand()) {
			...
			for (Player p : mainApp.GetSeatedPlayers())
			{
				...
				Hand bestHand = Hand.PickBestHand(AllHands);
				GPPH.setBestHand(bestHand);
				hsPlayerHand.put(bestHand, GPPH.getPlayer());
				BestPlayerHands.add(bestHand);//Goes through all the players grabs their hands and chooses the best using a function in Hand
			}
			Hand winner = Hand.PickBestHand(BestPlayerHands);
			Player winnerP = (Player) hsPlayerHand.get(winner);
			System.out.println("Winning Player Position: " + winnerP.getiPlayerPosition());
			SetGameControls(eGameState.EndOfGame);//Prints out winner and ends game
		} else 
		{
			if (cardDrawPlayer + cardDrawCommon + 2 >= gme.getRule().getTotalCardsToDraw()) 
			{
				for (Player p : mainApp.GetSeatedPlayers()) {
	...
	private SequentialTransition CalculateTransition(Card c, HBox PlayerCardBox, ImageView imView, int cardDraw) 
	{
		...
		ImageView imgvCardFaceDown = (ImageView) PlayerCardBox.getChildren().get(cardDraw - 1);
